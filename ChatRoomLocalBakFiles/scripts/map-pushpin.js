...
function rotatePushpin(pin, dest, url, angle) {
    console.log("pin icon path on rotate pins", pin.getIcon());    
    var img = new Image();
    img.onload = function () {
...
function addPins(aircraftList) {
    ...
    for (var i = 0; i < l; i++) {
        ...
        console.log("pin icon path on add pins", pin.getIcon());
...
function movePins(newAircraftList) {
    ...
    for (var i = 0; i < l; i++) {
        ...
        if (aircraftDict[key] != undefined) {
            aircraftDict[key].setOptions({visible: true});
            ...
            if (key == 'AAA224') {
                console.log(pin, 'from', pin.getLocation().latitude, pin.getLocation().longitude, 'to', newLat, newLong, newAircraftList[i]);
            }
